...
------ TRIM THE FEED TITLES FOR RAINMETER FORUMS
		 if Feeds[f].Title == 'Rainmeter Forums' then 
			...
			Item.ImageFile   = RawItem:match(Type.MatchImageFile)    or RawItem:match(Type.MatchImageFile2) or RawItem:match(Type.MatchImageFile3) or RawItem:match(Type.MatchImageFile4) or RawItem:match(Type.MatchImageFile5) or nil
         ...
         if (not Item.Title) or (Item.Title == '') then
            Item.Title = 'Untitled'
         elseif f == 6 then
            Item.Title = Item.Title:gsub('%s*%-%s*.-[iI]masters.*$', '')
         end
   ...
   if Error then
	...
	-- QUEUE ITEMS FOR DISPLAY
		for i = 1, math.max(#SortedFeed, MinItems) do
			...
			Queue['Item'..i..'ImageFile']    = Item.ImageFile   or ''
...
function DefineTypes()
   Types = {
	 ...
	 MatchImageFile = '<media:content width="460" url="(.-)"',
	 MatchImageFile2 = '<media:content.-url="(.-)"',
	 MatchImageFile3 = '<media:.-url="(.-)"',
	 MatchImageFile4 = '<enclosure.-url="(.-)"',
	 MatchImageFile5  = '<img.-src="(.-)"',
         MergeItems     = true,
